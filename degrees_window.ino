...
#define MIN_DEGREES_PER_BTN_PRESS 1
...
static int current_degree_step;
...
void recalc_degrees_positions() {
  ...
  float steps_per_btn_press = (float)settings.steps_per_revolution * degrees_per_btn_press / 360.0;
  Serial.printf("Number of steps per btn press: %.1f\n", steps_per_btn_press);

  // Fill the array with all tooth positions
  for (int i = 0; i < POSITIONS; i++) {
    degrees_positions[i] = steps_per_btn_press * i;
  }

  // Always reset after changing the number of teeth
  current_degree_step = 0;
...
void create_degrees_spinner() {
  ...
  lv_spinbox_set_range(degrees_spinner, MIN_DEGREES_PER_BTN_PRESS * DEGREE_DIVIDER, MAX_DEGREES_PER_BTN_PRESS * DEGREE_DIVIDER);
  lv_spinbox_set_digit_format(degrees_spinner, 4, 4 - log10(DEGREE_DIVIDER));
  lv_spinbox_step_prev(degrees_spinner);
  lv_obj_set_width(degrees_spinner, 80);
